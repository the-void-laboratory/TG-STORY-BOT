...
const OWNER_ID = parseInt(process.env.OWNER_ID, 10); // Only you can use owner features
const MONGODB_URI = process.env.MONGODB_URI;
...
// ── Global State Tracking Maps (Moved to top to prevent hoisting issues) ─────
const activeClients = new Map();     // userId -> TelegramClient instance
const pendingStories = new Map();    // userId -> pending story metadata
const waitingForCaption = new Set(); // userIds waiting to send a custom caption
const waitingForCustomTime = new Set(); // userIds waiting for custom minutes input
const userCooldowns = new Map();     // userId -> lastPostTimestamp
const loginStates = new Map();       // userId -> login progress state
...
bot.on("photo", async (msg) => {
  ...
  if (lastPostTime) {
    ...
    if (timeElapsed < COOLDOWN_SECONDS) {
      const remainingSeconds = Math.ceil(COOLDOWN_SECONDS - timeElapsed);
      return bot.sendMessage(msg.chat.id, `⏳ Please wait ${remainingSeconds} seconds before posting another story.`);
    }
  }
